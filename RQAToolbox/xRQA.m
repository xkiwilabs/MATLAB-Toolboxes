function xRQA(filename, norm, eDim, tLag, rescale, radius, doPlots, doStatsFile)
% **********************************************************************************************
%   function xRQA(filename, norm, eDim, tLag, rescale, radius, doPlots, doStatsFile)
%
%   Performs Cross Recurrence Quantification Analysis
%
%   Inputs:
%           filename   : data file; must be 2 column txt or csv file with no headers
...
%
%   Syntax Example:
%           xRQA('datafile.txt', 1, 3, 15, 1, 10, 1, 1)
%
%
% Output stats file columns: 
%           filename, eDim, tLag, rescale, radius, %REC, %DET, Maxline, MeanLine, Entropy
%           
...
tmin = 0; % do NOT remove line of identity for auto recurrence
...
%% Perform RQA stuff
ds  = xRQA_dist(DataX,DataY,eDim,tLag);
...
%
% Inputs:
%   dist -- square (NXN) distance matrix, w/distances only on the upper diagonal
...
%           Points inside the Theiler window (set by tmin) are set to 0 (nonrecurrent).
%
% Note: any errors in the input arguments causes the function
% to print an error message and return thrd = -1 (scalar).
%-----------------------------------------------------------------------------------
...
%   minl -- minimum line length (samples) for determinism, histogram, entropy, & complexity
%
% Outputs
%   rs -- structure containing the RQA stats
%   mats -- structure containing the big matrices (threshold distance, etc.)
%-----------------------------------------------------------------------------------

%% Threshold the distance matrix =========================================
%fprintf('Doing the thresholding...\n');
td = xRQA_radius(d,rescale,rad,tmin); %Threshold the distance matrix
if td == -1
